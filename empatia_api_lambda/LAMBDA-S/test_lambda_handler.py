...
class TestDetailsHappyPath:
    ...
    def test_valid_natural_s3_key_uses_detalle_prefix(self):
        # s3_key is no longer in the response — assert on the built key directly
        key = lambda_handler._build_s3_key("details", "test-id")
        assert "empatia/transcripciones/detalle/api/" in key

    def test_valid_natural_s3_key_has_hive_partitions(self):
        key = lambda_handler._build_s3_key("details", "test-id")
        assert re.search(r"year=\d{4}/month=\d{2}/day=\d{2}/", key)

    def test_s3_key_not_in_response(self):
        event = _make_event("/empatia/details/events", VALID_DETAILS_NATURAL)
        body = _body(lambda_handler.lambda_handler(event, {}))
...
class TestSummaryHappyPath:
    ...
    def test_valid_natural_s3_key_uses_resumen_prefix(self):
        key = lambda_handler._build_s3_key("summary", "test-id")
        assert "empatia/transcripciones/resumen/api/" in key
...
class TestAWSFailures:
    """
    Tests use BotoCoreError / ClientError — the specific exceptions the handler
    now catches instead of bare Exception (SonarQube S112 fix).
    """

    def test_sqs_failure_returns_500(self):
        mock_sqs.send_message.side_effect = BotoCoreError()
        event = _make_event("/empatia/details/events", VALID_DETAILS_NATURAL)
        resp = lambda_handler.lambda_handler(event, {})
    ...
    def test_sqs_client_error_returns_500(self):
    ...
    def test_s3_client_error_does_not_return_500(self):
        """ClientError on S3 is also non-fatal."""
...
class TestSecurityHardening:
    ...
    def test_oversized_body_returns_413(self):
        big = {**VALID_DETAILS_NATURAL}
        big["data"] = {"conversacion": "x" * (300 * 1024)}  # > 256 KB
        event = _make_event("/empatia/details/events", big)
        assert lambda_handler.lambda_handler(event, {})["statusCode"] == 413

    def test_response_has_nosniff_header(self):
        event = _make_event("/empatia/details/events", VALID_DETAILS_NATURAL)
        headers = lambda_handler.lambda_handler(event, {})["headers"]
    ...
    def test_long_tenant_id_is_truncated(self):
        assert len(lambda_handler._safe_tenant_id("a" * 500)) == 128

    def test_none_tenant_id_becomes_unknown(self):
        assert lambda_handler._safe_tenant_id(None) == "unknown"


# ===========================================================================
# 9 — CONVERSATION URL RESOLUTION
# ===========================================================================

class TestConversacionUrlResolution:

    def test_inline_text_is_left_unchanged(self):
        payload = {"data": {"conversacion": "Cliente: Hola\nAgente: Buenas"}}
        lambda_handler._resolve_conversacion(payload)
        assert payload["data"]["conversacion"] == "Cliente: Hola\nAgente: Buenas"

    def test_non_s3_host_is_refused(self):
        # SSRF guard: metadata endpoint style host must not be fetched
        result = lambda_handler._fetch_url_text("https://169.254.169.254/latest/meta-data/")
        assert result is None

    def test_http_scheme_is_refused(self):
        result = lambda_handler._fetch_url_text("http://bucket.s3.amazonaws.com/file.txt")
        assert result is None

    def test_s3_url_content_is_fetched_and_replaces_url(self):
        fake_text = "Cliente: Hola, necesito ayuda.\nAgente: Con gusto."
        ...
        class FakeResp:
            ...
            def read(self, n):
                return fake_text.encode("utf-8")

        url = "https://mybucket.s3.us-east-2.amazonaws.com/conv.txt?X-Amz-Signature=abc"
        with patch("urllib.request.urlopen", return_value=FakeResp()):
            payload = {"data": {"conversacion": url}}
            lambda_handler._resolve_conversacion(payload)

        assert payload["data"]["conversacion"] == fake_text

    def test_fetch_failure_keeps_original_url(self):
        url = "https://mybucket.s3.us-east-2.amazonaws.com/conv.txt?X-Amz-Signature=abc"
        with patch("urllib.request.urlopen", side_effect=OSError("boom")):
            payload = {"data": {"conversacion": url}}
            lambda_handler._resolve_conversacion(payload)

        # Non-fatal: URL kept as-is
        assert payload["data"]["conversacion"] == url

    def test_oversized_fetch_keeps_url(self):
        big = "x" * (300 * 1024)  # > MAX_FETCH_BYTES
